...
## Dose-response Curve Fitting and EC~10~ Calculation

### General

A tiered approach was used to estimate EC~10~ values. First, cells were treated with a broad dosage range to determine the region of interest for a later experiment with a narrower range. From this second experiment, final EC~10~ values were calculated. Dose-response modelling is done using the `drc_pipe()` function based on the `drc` package (@ritz_dose-response_2015).
...
drc_pipe <- function(dataframe, output_list, compound_name ="", lowest_conc_CI, highest_conc_CI, lower_limit_drm = c(NA, NA, NA, NA), upper_limit_drm = c(NA, NA, NA, NA), ec_level = 10, save_plot = F, plot_name = "plot.svg", show = "all", ylim_plot = c(0, NA), y_axis_breaks = seq(0, 150, 25)){
  ...
  drc_plot <- ggplot(modeled_df, aes(conc, V2)) +
    ...
    #geom_ribbon(aes(x = conc, y = V2, ymin = V3, ymax = V4), alpha = 0.2) +
    theme_bw()+
    annotation_logticks(sides = "b")+
    labs(title = compound_name, x = NULL, y = NULL)
    #labs(title = compound_name, x = expression(italic(c)~'/ µM'), y = "Relative viability / %")
  ...
  #output_list$dfs[[length(output_list$dfs)+1]] <- modeled_df
...
Plots and calculated EC~10~ values are appended to a pre existing list by the above function. This simple helper function creates a list with the correct formatting.
...
#### Experiments to estimate the EC~10~ more precisely
...
Estimated EC~10~ values used for metabolomics experiments are summarized in the table below:

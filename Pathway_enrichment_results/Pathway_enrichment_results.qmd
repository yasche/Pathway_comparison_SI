...
#ggraph(tft_pyrimidine_pathway, layout = "manual", x = x, y = y) +
#  geom_node_rect(aes(fill = -log10(p.value))) +
#  overlay_raw_map() +
#  #scale_fill_gradientn(colours = c("#001260", "white", "#590007"),
#  #                     values = scales::rescale(c(-40,0,5)),
#  #                     limits=c(-40, 5),
#  #                     na.value = NA) +
#  scale_fill_scico(palette = "lajolla", na.value = NA, direction = -1)+
#  theme_void()
...
```{r}
pyr_pathway <- parseKGML("output/moa_example/pyrimidine_pathway")
KEGGpathway2reactionGraph(pyr_pathway) %>%
  plot()
```
...
ggsave("output/plots/Fig1.png", width = 12, height = 15, dpi = 1200)
```
...
#| fig-width: 11
#| fig-height: 3.8
fig2_plota <- ggplot(corr_diff_meths_no_shave, aes(x, y, fill = r, label = round(r, digits = 2))) +
  geom_tile() +
...
ggsave("output/plots/corr_methods_all.png", fig2_plota, width = 11, height = 3.8)
```
...
Mean as a summary table:

```{r corr-distrib-all-tbl}
corr_diff_meths %>%
  select(CpdNoWrap, Target, x, y, r) %>%
  tidyr::unite(Methods, x, y, sep = "/") %>%
  group_by(Methods) %>%
...
#| fig-height: 3
#| fig-width: 3.7
fig2_plotc <- corr_diff_meths %>%
  select(CpdNoWrap, Target, x, y, r) %>%
  tidyr::unite(Methods, x, y, sep = "/") %>%
  ggplot(aes(Methods, r)) +
    geom_beeswarm(cex = 3, alpha = 0.4) +
    theme(axis.text.x = element_text(angle = 15, hjust = 1)) +
    ylab("\u03C1") +
    stat_summary(
          fun = mean, 
...
ggsave("output/plots/rho_dot_plot_mean_grey.png", fig2_plotc, width = 3.7, height = 3, dpi = 600)
```
...
fig2_plotb <- jac_diff_meths_matrix %>%
...
ggsave("output/plots/jdist_methods_matrix.png", fig2_plotb, width = 11, height = 3.8, dpi = 600)
```
...
```{r viz-jacc-methods-tbl}
jac_diff_meths %>%
  unnest(jacc_dists) %>%
  group_by(Methods) %>%
  summarise(Mean_J = mean(jDistance, na.rm = T))
```
...
ggsave("output/plots/jaccard_methods_grey.png", fig2_plotd, width = 3.7, height = 3, dpi = 600)
```
...
ggsave("output/plots/Fig2.png", width = 11, height = 11, dpi = 1200)
```
...
fig3_plota <- ggplot(corr_diff_cpds_no_shave, aes(interaction(str_wrap(x, width = 20, whitespace_only = F), str_wrap(Target_x, width = 20)), interaction(str_wrap(y, width = 20, whitespace_only = F), str_wrap(Target_y, width = 20)), fill = r, label = round(r, digits = 2))) +
  geom_tile()+
  geom_text(size = 3, color = "yellow") +
  facet_nested(~Method) +
...
ggsave("output/plots/corr_cpds_all.png", fig3_plota, width = 12, height = 6.8, dpi = 600)
```
...
```{r diff-cpds-corr-tbl}
corr_diff_cpds_corr_summary %>%
  group_by(Method, Similar_Mech) %>%
  summarise(Mean_r = round(mean(r, na.rm = T), digits = 2))
```
...
ggsave("output/plots/rho_dot_plot_meths_grey.png", width = 3.7, height = 3, dpi = 600)
```

The fact that one compound is compared to several others for "different" MOA probably violates the t test's independence criterion. Therefore, a version without p values is created:

```{r diff-cpds-corr-summary-plot-wo-signiflabel}
#| fig-height: 3
#| fig-width: 3.7

fig3_plotc <- corr_diff_cpds_corr_summary %>%
  #mutate(Method_signif = str_replace_all(Method, meths_signif_all)) %>%
  #mutate(Method_signif = paste0('italic(p)~"="~', Method_signif)) %>%
  mutate(MOA = case_when(Similar_Mech == TRUE ~ "similar",
                         Similar_Mech == FALSE ~ "different",
...
ggsave("output/plots/rho_dot_plot_meths_grey_wo_p.png", fig3_plotc, width = 3.7, height = 3, dpi = 600)
```
...
jacc_diff_cpds_signif_summary %>%
  group_by(Method, MOA) %>%
  summarise(mean_J = round(mean(jDistance, na.rm = T), digits = 2))
```
...
fig3_plotb <- jacc_diff_cpds_signif_summary_mirrored %>%
...
ggsave("output/plots/jdist_cpds_matrix.png", fig3_plotb, width = 12, height = 6.8, dpi = 600)
```
...
ggsave("output/plots/jaccard_cpds_grey.png", width = 3.7, height = 3, dpi = 600)
```
As above, it is not clear, wheter the criteria for t tests are met. Therefore, a version without p values is created.

```{r cpds-j-dotplot-wo-p}
#| fig-width: 3.7
#| fig-height: 3


fig3_plotd <- jacc_diff_cpds_signif_summary %>%
  #mutate(Method_signif = str_replace_all(Method, comps_jdist_signif)) %>%
  #mutate(Method_signif = paste0('italic(p)~"="~', Method_signif)) %>%
  mutate(MOA = case_when(MOA == TRUE ~ "similar",
                         MOA == FALSE ~ "different",
...
ggsave("output/plots/jaccard_cpds_grey.png", fig3_plotd, width = 3.7, height = 3, dpi = 600)
```

Create figure 3 for publication
...
wrap_plots(fig3_plota, 
           fig3_plotb,
           patchwork::free(fig3_plotc),
           plot_spacer(),
           patchwork::free(fig3_plotd),
           design = fig23_design,
           axes = "keep",
           heights = c(1, 1, 0.7, 0.7, 0.7),
           widths = c(1, 1, 0.1, 3.5)) +
  plot_annotation(tag_levels = "a")
...
wrap_plots(fig3_plota, 
           fig3_plotb,
           patchwork::free(fig3_plotc, type = "label"),
           #plot_spacer(),
           patchwork::free(fig3_plotd, type = "label"),
           design = fig23_design,
           axes = "keep",
           heights = c(1, 1, 1, 1)#,
           #widths = c(1, 1, 0.1, 3.5)
...
ggsave("output/plots/Fig3.png", width = 11.5, height = 15, dpi = 1200)
```
...
ggsave("output/plots/Fig4.png", width = 6.85, height = 4, dpi = 1200)
```
